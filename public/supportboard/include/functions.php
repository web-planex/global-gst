 ...
 * ==========================================================
 *
 * Main PHP functions files. © 2021 board.support. All rights reserved.
 *
 */

define('SB_VERSION', '3.1.9');
...
$sb_apps = ['dialogflow', 'slack', 'wordpress', 'tickets', 'woocommerce', 'ump', 'perfex', 'whmcs', 'aecommerce'];
...
function sb_external_db($action, $name, $query = '', $extra = false) {
    ...
    switch ($action) {
    	case 'connect':
            ...
            if (!defined('SB_' . $NAME . '_DB_NAME')) {
                $prefix = '';
                $database = sb_get_setting($name . '-db');
                if (empty($database[$name . '-db-name'])) {
                    echo 'Please insert the database details in the ' . $name . ' settings area.';
                    return false;
                }
            ...
            if ($connection->connect_error) {
                echo 'Exernal Database Connection error. Message: ' . $connection->connect_error . '.';
                return false;
            }
...
function sb_login($email = '', $password = '', $user_id = '', $user_token = '') {
    ...
    if ($valid_login) {
        $settings =  ['id' => $result['id'], 'profile_image' => $result['profile_image'], 'first_name' => $result['first_name'], 'last_name' => $result['last_name'], 'email' => $result['email'], 'user_type' => $result['user_type'], 'token' => $result['token'], 'url' => SB_URL];
...
function sb_get_active_user($login_data = false, $db = false, $login_app = false) {
    ...
    if ($login_app !== false) {
        ...
        if (defined('SB_PERFEX') && $app == 'perfex') {
            $return = sb_perfex_get_active_user_function($return, $login_app);
        }
        if (defined('SB_WHMCS') && $app == 'whmcs') {
            $return = sb_whmcs_get_active_user_function($return, $login_app);
        }
        if (defined('SB_AECOMMERCE') && $app == 'aecommerce') {
            $return = sb_aecommerce_get_active_user_function($return, $login_app);
        }
    }
...
function sb_js_admin() {
    ...
    $settings = [
        ...
        'routing' => (!$active_user || $active_user['user_type'] == 'agent') && (sb_get_multi_setting('queue', 'queue-active') || sb_get_setting('routing') || sb_get_setting('agent-hide-conversations')),
 ...
 * 12. Return all users, agents and admins
...
function sb_add_user($settings = [], $settings_extra = [], $hash_password = true) {
    ...
    if ($settings['password'] != '' && $hash_password) {
...
function sb_get_online_users($exclude_id = -1, $sorting = 'creation_time') {
    ...
    if (sb_pusher_active()) {
        ...
        $users = $ids == '' ? [] : sb_db_get(SELECT_FROM_USERS . ' FROM sb_users WHERE id IN (' . substr($ids, 0, -1) . ') ORDER BY ' . $sorting . ' DESC', false);
    } else {
...
function sb_is_user_online($user_id) {
    ...
    if (!$response && defined('SB_SLACK') && isset($user['user_type']) && sb_is_agent($user['user_type'])) {
        $response = sb_slack_presence($_POST['user_id']) == 'online';
    }
 ...
 * 14. Save a conversation as a CSV file
 *
 */
...
function sb_send_message($sender_id = -1, $conversation_id, $message = '', $attachments = [], $conversation_status_code = -1, $payload = false, $queue = false) {
    $pusher = sb_pusher_active();
    $push_notifications = defined('SB_API') && sb_get_multi_setting('push-notifications', 'push-notifications-active');
    $user_id = $sender_id;
    if ($sender_id == -1) {
        $sender_id = sb_isset(sb_get_active_user(), 'id');
    }
    if ($sender_id != -1) {
        $attachments_json = '';
        ...
        if (!$security || $pusher || $push_notifications) {
            ...
            if ($user_id == sb_get_active_user()['id']) {
                $security = true;
            }
        }
        if ($security) {
            ...
            $message = htmlspecialchars(sb_merge_fields($message), ENT_NOQUOTES);
            $response = sb_db_query('INSERT INTO sb_messages(user_id, message, status_code, creation_time, attachments, conversation_id, payload) VALUES ("' . $sender_id . '", "' . sb_db_escape($message) . '", 0, "' . gmdate('Y-m-d H:i:s') . '", "' . $attachments_json . '", "' . $conversation_id . '", "' . ($payload != false ? sb_db_json_enconde($payload) : '') . '")', true);
            ...
            if ($push_notifications) {
                ...
                if ($sender) {
                    sb_push_notification(sb_get_user_name($sender), $message, strpos($sender['profile_image'], 'user.svg') ? sb_get_setting('notifications-icon', SB_URL . '/media/icon.png') : $sender['profile_image'], 'agents', $conversation_id);
                }
            }
...
function sb_update_or_delete_message($action, $message_id, $message = false, $attachments = false, $payload = false) {
    ...
    if ($security) {
        if ($action == 'update') {
            ...
            $response = sb_db_query('UPDATE sb_messages SET ' . ($message !== false ? 'message = "' . sb_db_escape($message) . '",' : '') . ' creation_time = "' . gmdate('Y-m-d H:i:s') . '"' . ($payload !== false ? ', payload = "' . sb_db_json_enconde($payload) . '"' : '') . ($attachments !== false ? ', attachments = "' . sb_db_json_enconde($attachments) . '"' : '') . ' WHERE id = "' . $message_id . '" LIMIT 1');
        }
        if ($action == 'delete') {
...
function sb_is_agent_typing($conversation_id) {
    return sb_db_get('SELECT id, first_name, last_name FROM sb_users WHERE typing = ' . $conversation_id . ' AND (user_type = "agent" OR user_type = "admin") AND id <> ' . sb_get_active_user()['id']);
}
 ...
 * 6. Save a translation langauge file and a copy of it as backup
 * 7. Restore a translation language file from a backup
 * 8. Return the user langauge code
 *
 */
...
function sb_init_translations() {
    ...
    if (!empty($SB_LANGUAGE) && $SB_LANGUAGE[0] !='en') {
        ...
        $admin = sb_is_agent();
        $admin_lang = $admin && defined('SB_ADMIN_LANG') ? SB_ADMIN_LANG : false;
 ...
 * 11. Return the settings of the front-end
 * 12. Return the setting of block message
 * 13. Return the HTML code of the color palette
 *
 */
...
function sb_save_external_setting($name, $value) {
    ...
    return JSON_ERROR_NONE !== json_last_error() ? json_last_error_msg() : sb_db_query('INSERT INTO sb_settings(name, value) VALUES (\'' . $name . '\', \'' . $settings_encoded . '\') ON DUPLICATE KEY UPDATE value = \'' . $settings_encoded . '\'');
}
...
function sb_get_all_settings() {
    $settings = [];
    $result = sb_db_get('SELECT value FROM sb_settings WHERE name="emails" || name="rich-messages" || name="wc-emails"', false);
...
function sb_get_front_settings() {
    ...
    if ($active_user) {
        ...
        if (!sb_is_agent($active_user)) {
            if (isset($_SERVER['HTTP_REFERER'])) sb_current_url($user_id, $_SERVER['HTTP_REFERER']);
...
function sb_get_block_setting($value) {
    switch ($value)  {
        ...
        case 'welcome':
            ...
            return ['active' => $settings['welcome-active'], 'message' => sb_rich_value($settings['welcome-msg']), 'open' => $settings['welcome-open'], 'sound' => $settings['welcome-sound']];
        case 'follow':
            $settings = sb_get_setting('follow-message');
            return ['active' => $settings['follow-active'], 'title' => sb_rich_value($settings['follow-title']), 'message' => sb_rich_value($settings['follow-msg']), 'name' => $settings['follow-name'] ? 'true' : 'false', 'last-name' => sb_isset($settings, 'follow-last-name') ? 'true' : 'false', 'success' => sb_rich_value($settings['follow-success']), 'placeholder' => sb_rich_value(sb_isset($settings, 'follow-placeholder', 'Email'), false), 'delay' => sb_isset($settings, 'follow-delay')];
        case 'subscribe':
    ...
    if ($articles != false) {
        ...
        if ($id == -1) {
            ...
            for ($i = 0; $i < $articles_count; $i++) {
                if ($articles[$i]['id'] == $id) {
                    sb_reports_update('articles-views', false, false, $id);
                    return $articles[$i];
                }
            }
            return false;
 ...
 * 9. Convert the text formatting of Support Board to HTML
 *
 */
...
function sb_email_create($recipient_id, $sender_name, $sender_profile_image, $message, $attachments = [], $department = false, $conversation_id = false) {
    ...
    $settings = sb_get_external_setting('emails')['email-' . $suffix][0];
...
function sb_email_create_content($subject, $message, $attachments, $replacements) {
    ...
    $message = str_replace(['{recipient_name}', '{sender_name}', '{sender_profile_image}', '{message}', '{attachments}', '{conversation_link}'], [$replacements['recipient_name'], $replacements['sender_name'],  $replacements['sender_profile_image'], $replacements['message'], $attachments_code, (SB_URL . '/admin.php' . (isset($replacements['conversation_id']) ? '?conversation=' . $replacements['conversation_id'] : ''))], $message);
...
function sb_email($recipient_id, $message, $attachments = [], $sender_id = -1) {
    ...
    if ($sender != false && isset($sender['id']) && $user != false && isset($user['id'])) {
        ...
        $emails = sb_get_external_setting('emails')['email-' . $email_type][0];
...
function sb_email_send_test($to, $email_type) {
    ...
    $settings = sb_get_external_setting('emails')['email-' . $email_type][0];
                ...
                foreach($emails as $email_number) {
                    ...
                    if (strpos($to, '| SB')) {
                        ...
                        if (!in_array($id, $history)) {
                            ...
                            $sender = sb_db_get('SELECT * FROM sb_users WHERE email = "' . (strpos($from, '<') ? substr($from, strpos($from, '<') + 1, -1) : $from) . '" LIMIT 1');
                            if ($sender) {
                                $message = quoted_printable_decode(imap_fetchbody($inbox, $email_number, 2));
                                $agent = $sender['user_type'] == 'agent' || $sender['user_type'] == 'admin';
                                if (strpos($host, 'gmail') && strpos($message, 'gmail_quote')) {
                                    $message = substr($message, 0, strpos($message, 'gmail_quote'));
                                    $message = str_replace('<div>', PHP_EOL, $message);
                                }
                                if ($settings['email-piping-delimiter'] && strpos($message, '### ')) {
                                    $message = substr($message, 0, strpos($message, '### '));
                                }
                                $message = strip_tags($message);
                                if (!empty($message)) {
                                    $recipient = get_user_from_conversation($conversation_id, !$agent);
                                    sb_send_message($sender['id'], $conversation_id, utf8_encode($message), [], ($agent ? 1 : 2));
                                    if (isset($recipient['id']) && (($agent && sb_get_setting('notify-user-email')) || (!$agent) && sb_get_setting('notify-agent-email')) && !sb_is_user_online($recipient['id'])) {
                                        sb_email_create($recipient['id'], sb_get_user_name($sender), $sender['profile_image'], $message, [], false, $conversation_id);
                                    }
...
function sb_subscribe_email($email) {
    $settings = sb_isset(sb_get_external_setting('emails'), 'email-subscribe');
    $subject = $settings[0]['email-subscribe-subject'][0];
    $content = $settings[0]['email-subscribe-content'][0];
...
function sb_installation($details) {
    ...
    if ($connection_check === true) {
        ...
        $db_respones['reports'] = $connection->query('CREATE TABLE IF NOT EXISTS sb_reports (name VARCHAR(255) NOT NULL, value VARCHAR(255) NOT NULL, creation_time DATE NOT NULL, external_id INT, extra VARCHAR(255), UNIQUE INDEX sb_reports_data_index (name, value, date_time)) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci');
...
function sb_updates_check() {
    if (!isset($_COOKIE['sb-updates']) || $_COOKIE['sb-updates'] != SB_VERSION) {
        ...
        sb_db_query('CREATE TABLE IF NOT EXISTS sb_reports (name VARCHAR(255) NOT NULL, value VARCHAR(255) NOT NULL, creation_time DATE NOT NULL, external_id INT, extra VARCHAR(255), UNIQUE INDEX sb_reports_data_index (name, value, creation_time)) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci');
...
function sb_push_notification($title = '', $message = '', $icon = '', $interest = false, $conversation_id = false) {
    if (is_numeric($interest) || is_array($interest)) {
        if (!sb_is_agent()) {
            ...
            if (is_numeric($interest)) {
                if (!in_array(intval($interest), $agents_ids)) return new SBError('security-error', 'sb_push_notification');
            } else {
    ...
    if (sb_is_agent()) {
        $link = $conversation_id ? sb_isset(sb_db_get('SELECT B.value FROM sb_conversations A, sb_users_data B WHERE A.id = ' . $conversation_id . ' AND A.user_id = B.user_id AND B.slug = "current_url" LIMIT 1'), 'value', '') : '';
    } else {
        $link = SB_URL . '/admin.php';
    }
    $instance_ID = sb_get_multi_setting('push-notifications', 'push-notifications-id');
    $query = '{"interests":["' . $interest . '"],"web":{"notification":{"title":"' . str_replace('"', '', $title) . '","body":"' . str_replace('"', '', $message) . '","icon":"' . $icon . '","deep_link":"' . $link . '","hide_notification_if_site_has_focus":true}, "data": {"conversation_id":"' . $conversation_id . '","user_id":"' . sb_get_active_user()['id'] . '"}}}';
...
function sb_pusher_trigger($channel, $event, $data = []) {
    ...
    $user_id = sb_isset(sb_get_active_user(), 'id');
    ...
    return $security ? $pusher->trigger($channel, $event, $data) : new SBError('pusher-security-error', 'sb_pusher_trigger');
}
...
function sb_pusher_agents_online() {
    $online = false;
    $agents = sb_db_get('SELECT id FROM sb_users WHERE user_type = "agent" OR user_type = "admin"', false);
    $agents_id = [];
    for ($i = 0; $i < count($agents); $i++) {
        array_push($agents_id, $agents[$i]['id']);
    }
    $users = sb_pusher_get_online_users();
    for ($i = 0; $i < count($users); $i++) {
 ...
 * 10. Debug function
 * 11. Convert a JSON string to an array
 *
 */
...
function sb_encryption($action, $string) {
    ...
    $secret_key = sb_get_setting('envato-purchase-code', 'supportboard');
...
function sb_curl($url, $post_fields = '', $header = [], $type = 'POST') {
    ...
    switch ($type){
        ...
        case 'FILE':
            ...
            curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
            $file = fopen(sb_upload_path() . '/' . basename($url), 'wb');
 ...
 * 2. Check if the current time is within the office hours
 * 3. Generate the CSS with values setted in the settings area
 * 4. Delete visitors older than 24h, messages in trash older than 30 days. Archive conversation older than 24h with status code equal to 4 (pending user reply).
 * 5. Check the system for requirements and issues
 *
 */
...
function sb_reports($report_name, $date_start = false, $date_end = false) {
    ...
    switch ($report_name) {
        ...
        case 'missed-conversations':
            ...
            $sum = !in_array($report_name, ['visitors', 'subscribe', 'follow-up', 'registrations']);
            ...
            if ($single) {
                ...
                for ($i = 0; $i < count($articles); $i++) {
                    ...
                    if (isset($data[$id])) {
                        ...
                        $data_names[strlen($article_title) > 50 ? substr($article_title, 0, 50) . '...' : $article_title] = $data[$id];
                    }
                }
                $data = $data_names;
            }
        ...
        case 'agents-ratings':
            $ratings = sb_get_external_setting('ratings');
            if ($ratings) {
                $rows = sb_db_get('SELECT id, first_name, last_name FROM sb_users WHERE user_type = "agent" OR user_type = "admin"', false);
                $agent_names = [];
                for ($i = 0; $i < count($rows); $i++) {
                    $agent_names[$rows[$i]['id']] = $rows[$i]['first_name'] . ' ' . $rows[$i]['last_name'];
                }
                ...
                foreach ($data as $key => $value) {
                    $positive = $data[$key][0];
                    $negative = $data[$key][1];
                    $average = abs(round(100 - (abs($negative - $positive) / (($negative + $positive) / 2)) * 100, 2));
                    $data[$key] = [$average, '<i class="sb-icon-check"></i>' . $positive . ' (' . $average . '%) <i class="sb-icon-close"></i>' . $negative];
                }
            }
            break;
        case 'os':
        ...
        case 'countries':
            ...
            for ($i = 0; $i < count($rows); $i++) {
                $value = $rows[$i]['value'];
                $valid = false;
                if ($report_name == 'countries' && strpos($value, ',')) {
                ...
                if ($is_languages && isset($language_codes[strtolower($value)])) {
                    ...
                    if (!isset($flags[$value]) && file_exists(SB_PATH . '/media/flags/' . $language_code . '.png')) {
            ...
            foreach ($data as $key => $value) {
                $image = '';
                if (isset($flags[$key])) $image = '<img src="' . SB_URL . '/media/flags/' . $flags[$key] . '.png" />';
                if ($is_browser) {
...
function sb_reports_update($name, $value = false, $external_id = false, $extra = false) {
    ...
    switch ($name) {
        ...
        case 'subscribe':
        case 'visitors':
            return sb_db_query('INSERT INTO sb_reports VALUES ("' . $name . '", 1, "' . $now . '", NULL, NULL) ON DUPLICATE KEY UPDATE value = value + 1');
        case 'articles-searches':
            return sb_db_query('INSERT INTO sb_reports VALUES ("articles-searches", "' . sb_db_escape($value) . '", "' . $now . '", NULL, NULL)');
        case 'articles-views':
            $where = ' WHERE name = "articles-views" AND extra = "' . $extra . '" AND creation_time = "' . $now . '"';
            $views = sb_db_get('SELECT value FROM sb_reports' . $where);
            return sb_db_query(empty($views) ? 'INSERT INTO sb_reports VALUES ("articles-views", 1, "' . $now . '", NULL, "' . $extra . '")' : 'UPDATE sb_reports SET value = ' . (intval($views) + 1) . $where);
    }
    return false;
}
?>