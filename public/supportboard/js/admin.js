    ...
    var SBApps = {
        ...
        aecommerce: {
            ...
            conversationPanel: function () {
                ...
                if (!$(this.panel).sbLoading() && aecommerce_id != '') {
                    SBF.ajax({
        ...
        whmcs: {
            ...
            conversationPanel: function () {
                ...
                if (!$(this.panel).sbLoading() && whmcs_id != '') {
                    SBF.ajax({
                        ...
                        code += `<a href="${SB_ADMIN_SETTINGS['whmcs-url']}/admin/clientssummary.php?userid=${whmcs_id['value']}" target="_blank" class="sb-btn sb-whmcs-link">${sb_('View on WHMCS')}</a>`;
                        $(this.panel).html(code).sbLoading(false);
                        collapse(this.panel, 160);
                    });
                    $(this.panel).sbLoading(true);
                    ...
                    case 'tab-articles':
                        if (articles.length) {
                            ...
                            SBF.ajax({
                                ...
                                showResponse(response === true ? 'Articles saved' : response);
                                $(btn).sbLoading(false);
                            });
                        } else {
                            $(btn).sbLoading(false);
                        ...
                        yAxes: [{
                            ticks: {
                                callback: function (tickValue, index, ticks) {
                                    return label_type == 1 ? tickValue : (label_type == 2 ? new Date(tickValue * 1000).toISOString().substr(11, 8) : label_type);
                                },
                                beginAtZero: true
                            }
                        }],
    ...
    var SBUsers = {
        ...
        getRow: function (user) {
            if (user instanceof SBUser) {
                return `<tr data-user-id="${user.id}" data-user-type="${user.type}"><td><input type="checkbox" /></td><td class="sb-td-profile"><a class="sb-profile"><img src="${user.image}" /><span>${user.get('full_name')}</span></a></td><td class="sb-td-email">${user.get('email')}</td><td class="sb-td-ut">${user.type}</td><td>${SBF.beautifyTime(user.get('last_activity'), true)}</td><td>${user.get('creation_time')}</td></tr>`;
            } else {
                    ...
                    if (response.length) {
                        ...
                        for (var i = 0; i < response.length; i++) {
                            if (!id_check.includes(response[i]['conversation_id'])) {
                                ...
                                // Update user
                                if (('event' in payload && payload['event'] == 'update-user') || (users[user_id].type != item['user_type'])) {
                                    activeUser().update(() => {
                                        this.updateUserDetails();
    ...
    $(document).ready(function () {
        ...
        $(header).on('click', '.sb-version', function () {
            ...
            SBF.ajax({
                ...
                let code = '';
                let names = { 'sb': 'Support Board', 'slack': 'Slack', 'dialogflow': 'Dialogflow', 'tickets': 'Tickets', 'woocommerce': 'Woocommerce', 'ump': 'Ultimate Membership Pro', 'perfex': 'Perfex', 'whmcs': 'WHMCS', 'aecommerce': 'Active eCommerce' };
                let updates = false;
        ...
        */
        ...
        // Synchronize users and articles
