        ...
        init: function (onSuccess = false) {
            if (SBPusher.active) {
                ...
                $.getScript('https://js.pusher.com/7.0/pusher.min.js', () => {
                    this.pusher = new Pusher(admin ? SB_ADMIN_SETTINGS['pusher-key'] : CHAT_SETTINGS['pusher-key'], { cluster: admin ? SB_ADMIN_SETTINGS['pusher-cluster'] : CHAT_SETTINGS['pusher-cluster'], authEndpoint: SB_URL + '/include/pusher.php' });
            ...
            if (this.followUpCheck()) {
                ...
                timeout = setTimeout(() => {
                    if (this.followUpCheck()) {
                        ...
                        this.sendMessage(bot_id, `[email id="sb-follow-up-form" title="${settings['title']}" message="${settings['message']}" placeholder="${settings['placeholder']}" name="${settings['name']}" last-name="${settings['last-name']}" success="${settings['success']}"]`);
                        this.scrollBottom();
    ...
    function initialize() {
        ...
        if (main.length && typeof SB_AJAX_URL != ND) {
            ...
            SBF.ajax({
                ...
                if (!admin && !CHAT_SETTINGS['chat-manual-init'] && (!CHAT_SETTINGS['disable-offline'] || agents_online) && (!CHAT_SETTINGS['disable-office-hours'] || CHAT_SETTINGS['office-hours']) && (!CHAT_SETTINGS['chat-login-init'] || SBApps.login())) {
                    SBChat.initChat();
                }
