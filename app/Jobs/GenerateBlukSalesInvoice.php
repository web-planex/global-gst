...
class GenerateBlukSalesInvoice implements ShouldQueue
{
    ...
    public function createMultiInvoicePDF() {
        ...
        foreach($this->checkboxes as $in_id){
            ...
            $data['name'] = 'Sales Invoice';
            $data['content'] = 'This is sales invoice pdf.';
            ...
            $path = $this->user->id.'/sales_invoice/';
            $root = base_path() . '/public/upload/' . $path;
            if (!file_exists($root)) {
                mkdir($root, 0777, true);
            }
            if(!$pdf->saveAs(public_path().'/upload/'.$path.'sales_invoice_'.$in_id.'.pdf')){
                $error = $pdf->getError();
                Log::error($error);
            } else {
                $files[] = public_path().'/upload/'.$path.'sales_invoice_'.$in_id.'.pdf';
            }
        }

        $zip_name = 'sales_invoice_multiple_pdf_'.time().'.zip';
        $zip_path = public_path() .'/upload/'.$this->user->id.'/sales_invoice/'.$zip_name;
        $zip = new \ZipArchive;
    ...
    public function createSignleInvoicePDF() {
        ...
        foreach($this->checkboxes as $id){
            ...
            $pdf->addPage(view('globals.invoice.template.template_'.$data['company']['pdf_template'],[
                'invoice' => $invoice,
                'taxes' => $taxes,
                'user' => $user,
        ...
        $zip_name = 'sales_invoice_single_pdf_'.time().'.zip';
        $zip_path = public_path() .'/upload/'.$this->user->id.'/sales_invoice/'.$zip_name;
        $zip = new \ZipArchive;
        $zip->open($zip_path, \ZipArchive::CREATE);
        foreach ($files as $file) {
